...
module.exports = function(config, callback){
	...
	var baseConfig = {};
	if(config.file){
		baseConfig = fileHandler(config.file, config);
...
function resolveFiles(config){
	var basePath;
	if(config.file){
		basePath = path.dirname(path.resolve(config.file));
	} else {
		basePath = __dirname;
	}

	utils.each(config.serve, function(filePath, key, map){
		map[key] = path.resolve(basePath, filePath);
	});

	if(typeof config.run === "string"){
		config.run = path.resolve(basePath, config.run);
	} else {
		utils.each(config.serve, function(filePath, index, arr){
			arr[index] = path.resolve(basePath, filePath);
		});	
	}
}
...
function autoAdaptFileList(fileList){
	var adaptableLibraryRegexMap = utils.map(adapters, function(name){
...
function fileHandler(file, config){
	var baseConfig = {};

	try {
		vm.runInNewContext(fs.readFileSync(file), baseConfig);
		...
		if(Array.isArray(baseConfig.run)){
			baseConfig.run = baseConfig.run.map(function(filePath){
				return path.resolve(path.dirname(path.resolve(file)), filePath);
			});
		}
