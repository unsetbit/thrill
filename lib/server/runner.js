...
var runner = module.exports = function(config, callback){
	...
	if(!config.run){
		callback(new Error("[Thrill] Unable to find the required run variable in the configuration, aborting."));
		return;
	}
	...
	function onThrillReady(thrill){
		...
		if(thrill instanceof Error){
			...
			return callback(thrill);
		}

		// If auto adapt is enabled, auto inject the adapters
		// HTML files are auto adapted on the fly with the HTTP server
		...
		// If we have an html and don't have a serve directory, guess the best root directory
		// to serve this file
		if(!config.serve && typeof config.run === "string"){
			var contents = fs.readFileSync(config.run).toString();

			// This finds relative paths leading to parent directories
			// We're going to find the furthest-most parent with it and use that as the
			// root directory for this html file.
			var rootFinder = /(?:src=|href=)(?:["|']){1}((?:\.{1,2}\/)*)*/gi,
				rootMatch,
				biggestMatch = ".";

			// Go through the entire file, trying to find the furthest-most parent
			// of this html file that it utilizes
			while(rootMatch = rootFinder.exec(contents)){
				if(!rootMatch[1]) continue;
				
				// Since we're only looking at ./../../../ patterns, it's ok
				// to use length as a determining factor of level
				if(rootMatch[1].length > biggestMatch.length){
					biggestMatch = rootMatch[1];
				}
			}
			var rootDir = path.resolve(path.dirname(config.run), biggestMatch);
			debug("[Thrill] Guessing that the root directory for this HTML file is " + rootDir + "\n");
			config.serve = {"" : rootDir};
		}
