...
exports.HttpServer = HttpServer = function(httpServer, expressInstance, host){
...
HttpServer.prototype.kill = function(callback){
	this.connections.forEach(connection.destroy.bind(connection));
...
var adapterRegexMap = utils.map(adapters, function(name){
	return new RegExp('<script.*src=[\'"]?.*thrill-' + name + '-adapter.*\\.js[\'"].*?>.*</.*script.*>', 'im');
});

HttpServer.prototype.serveAdaptedHtmlFile = function(request, response, filePath){
	var self = this;
...
HttpServer.prototype.serve = function(pathMap, run){
	...
	function serveFile(filePath){
		...
		resourcePath = path.basename(filePath);
