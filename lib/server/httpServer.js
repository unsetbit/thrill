...
var create = module.exports = function(callback, options){
	...
	if(options.autoAdaptLibraries !== void 0) server.autoAdaptLibraries = options.autoAdaptLibraries;
	if(options.autoAdaptExtensions) server.autoAdaptExtensions = options.autoAdaptExtensions;
	
	return server;
};

exports.HttpServer = HttpServer = function(server, host, httpServer){
...
HttpServer.prototype.autoAdaptLibraries = true;
...
HttpServer.prototype.serve = function(pathMap, run){
	...
	if(runIsString){ 
		...
		utils.each(groupResources, function(value, key){
			if(~run.indexOf(key)) runFileIsServed = true;
		});
